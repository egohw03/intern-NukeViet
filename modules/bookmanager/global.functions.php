...
function nv_get_books($limit = 10, $offset = 0)
{
    ...
    $books = array();
    $sql = 'SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_books WHERE status = 1 ORDER BY add_time DESC LIMIT ' . (int)$limit . ' OFFSET ' . (int)$offset;
    $result = $db->query($sql);
...
function nv_count_books()
{
    ...
    $stmt = $db->prepare('SELECT COUNT(*) FROM ' . NV_PREFIXLANG . '_' . $module_data . '_books WHERE status = 1');
    $stmt->execute();
